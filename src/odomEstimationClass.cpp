...
void OdomEstimationClass::updatePointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& edge_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr& surf_in , bool clear_map, double edge_limit, double surf_limit){
    ...
    if(laserCloudCornerMap->points.size()>10 && laserCloudSurfMap->points.size()>50){
        ...
        for (int iterCount = 0; iterCount < 2; iterCount++){
            ceres::LossFunction *loss_function = new ceres::SoftLOneLoss(0.1);
    ...
    addPointsToMap(downsampledEdgeCloud,downsampledSurfCloud , clear_map, edge_limit, surf_limit);
...
void OdomEstimationClass::addPointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& downsampledEdgeCloud, const pcl::PointCloud<pcl::PointXYZI>::Ptr& downsampledSurfCloud, bool clear_map, double edge, double surf_limit){


    if(clear_map){
        if(laserCloudCornerMap->size()>edge){
            //occlude_pcd(laserCloudCornerMap,1,0,0);
           laserCloudCornerMap->clear();
       }

       if(laserCloudSurfMap->size()>surf_limit){
           //occlude_pcd(laserCloudSurfMap,1,0,0);
...
void OdomEstimationClass::occlude_pcd(pcl::PointCloud<pcl::PointXYZI>::Ptr & cld_ptr,int dim, double threshA, double threshB)
{
    for(pcl::PointCloud<pcl::PointXYZI>::iterator it = cld_ptr->begin()+threshA;  it < cld_ptr->end()-threshB;it+=dim)
    {
        cld_ptr->erase(it);
